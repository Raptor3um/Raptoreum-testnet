...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the RaptoreumTestFramwork, place overrides for the
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to control whether
  ...
  has 25 mature blocks (25x5000=125000 RTM) in its wallet.
...
maintain using the standard RaptoreumTestFramework.
...
Taken from the [python-raptoreumrpc repository](https://github.com/jgarzik/python-raptoreumrpc).
...
Utilities for manipulating transaction scripts (originally from python-raptoreumlib)

#### [test_framework/blockstore.py](test_framework/blockstore.py)
Implements disk-backed block and tx storage.

#### [test_framework/key.py](test_framework/key.py)
Wrapper around OpenSSL EC_Key (originally from python-raptoreumlib)
