                                    ...
                                    unsigned int nIn, unsigned int flags, raptoreumconsensus_error* err)
{
    ...
    try {
        ...
        if (nIn >= tx.vin.size())
            return set_error(err, raptoreumconsensus_ERR_TX_INDEX);
        if (GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION) != txToLen)
            return set_error(err, raptoreumconsensus_ERR_TX_SIZE_MISMATCH);
...
int raptoreumconsensus_verify_script_with_amount(const unsigned char *scriptPubKey, unsigned int scriptPubKeyLen, int64_t amount,
                                    const unsigned char *txTo        , unsigned int txToLen,
                                    unsigned int nIn, unsigned int flags, raptoreumconsensus_error* err)
{
    CAmount am(amount);
                                   ...
                                   unsigned int nIn, unsigned int flags, raptoreumconsensus_error* err)
{
    if (flags & raptoreumconsensus_SCRIPT_FLAGS_VERIFY_WITNESS) {
        return set_error(err, raptoreumconsensus_ERR_AMOUNT_REQUIRED);
    }
