...
#ifndef RAPTOREUM_RAPTOREUMCONSENSUS_H
#define RAPTOREUM_RAPTOREUMCONSENSUS_H

#include <stdint.h>

#if defined(BUILD_RAPTOREUM_INTERNAL) && defined(HAVE_CONFIG_H)
...
#define RAPTOREUMCONSENSUS_API_VER 1
...
typedef enum raptoreumconsensus_error_t
{
    ...
    raptoreumconsensus_ERR_AMOUNT_REQUIRED,
...
enum
{
    ...
    raptoreumconsensus_SCRIPT_FLAGS_VERIFY_WITNESS             = (1U << 11), // enable WITNESS (BIP141)
    raptoreumconsensus_SCRIPT_FLAGS_VERIFY_ALL                 = raptoreumconsensus_SCRIPT_FLAGS_VERIFY_P2SH | raptoreumconsensus_SCRIPT_FLAGS_VERIFY_DERSIG |
                                                               ...
                                                               raptoreumconsensus_SCRIPT_FLAGS_VERIFY_CHECKSEQUENCEVERIFY | raptoreumconsensus_SCRIPT_FLAGS_VERIFY_WITNESS
};
...
EXPORT_SYMBOL int raptoreumconsensus_verify_script_with_amount(const unsigned char *scriptPubKey, unsigned int scriptPubKeyLen, int64_t amount,
                                    const unsigned char *txTo        , unsigned int txToLen,
                                    unsigned int nIn, unsigned int flags, raptoreumconsensus_error* err);
...
#endif // RAPTOREUM_RAPTOREUMCONSENSUS_H
