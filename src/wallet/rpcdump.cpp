...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    return CRaptoreumSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
     ...
     * https://github.com/RaptoreumProject/Raptoreumcoin/issues/9934
    ...
    // produce output
    file << strprintf("# Wallet dump created by Raptoreum %s\n", CLIENT_BUILD);
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        if (pwallet->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CRaptoreumSecret(key).ToString(), strTime);
