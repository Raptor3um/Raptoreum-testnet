...
const std::string RaptoreumGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString RaptoreumGUI::DEFAULT_WALLET = "~Default";

RaptoreumGUI::RaptoreumGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void RaptoreumGUI::createActions()
{
    QFont font = QFont();
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to RTM addresses"));
...
void RaptoreumGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void RaptoreumGUI::createToolBars()
{
    if(walletFrame)
    {
        /** RTM START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/raptoreumcointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* raptoreumLabelLayout = new QVBoxLayout(toolbarWidget);
        raptoreumLabelLayout->addWidget(label);
        raptoreumLabelLayout->addWidget(toolbar);
        raptoreumLabelLayout->setDirection(QBoxLayout::TopToBottom);
        raptoreumLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Raptoreumcoin Market Price"));
...
void RaptoreumGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool RaptoreumGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool RaptoreumGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void RaptoreumGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void RaptoreumGUI::setWalletActionsEnabled(bool enabled)
{
...
void RaptoreumGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void RaptoreumGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void RaptoreumGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void RaptoreumGUI::optionsClicked()
{
...
void RaptoreumGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void RaptoreumGUI::showDebugWindow()
{
...
void RaptoreumGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void RaptoreumGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void RaptoreumGUI::openClicked()
{
...
void RaptoreumGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void RaptoreumGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void RaptoreumGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void RaptoreumGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void RaptoreumGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void RaptoreumGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** RTM START */
void RaptoreumGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void RaptoreumGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void RaptoreumGUI::gotoManageAssetsPage()
{
...
void RaptoreumGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Raptoreum network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void RaptoreumGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void RaptoreumGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void RaptoreumGUI::updateHeadersSyncProgressLabel()
{
...
void RaptoreumGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void RaptoreumGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Raptoreum"); // default title
...
void RaptoreumGUI::changeEvent(QEvent *e)
{
...
void RaptoreumGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void RaptoreumGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void RaptoreumGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "RTM")
        msg += tr("Amount: %1\n").arg(RaptoreumUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(RaptoreumUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void RaptoreumGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void RaptoreumGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool RaptoreumGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool RaptoreumGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void RaptoreumGUI::setHDStatus(int hdEnabled)
{
...
void RaptoreumGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void RaptoreumGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void RaptoreumGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void RaptoreumGUI::detectShutdown()
{
...
void RaptoreumGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void RaptoreumGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void RaptoreumGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(RaptoreumGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void RaptoreumGUI::subscribeToCoreSignals()
{
...
void RaptoreumGUI::unsubscribeFromCoreSignals()
{
...
void RaptoreumGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<RaptoreumUnits::Unit> units = RaptoreumUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const RaptoreumUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(RaptoreumUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (RaptoreumUnits::Unit u : RaptoreumUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(RaptoreumUnits::name(u)), this);
...
{
    setText(RaptoreumUnits::name(newUnits));
}
...
void RaptoreumGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=RTMBTC"));
