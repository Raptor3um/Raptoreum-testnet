...
The RPM spec file provided here is for Raptoreum-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://raptoreum.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Raptoreum git repository](https://github.com/RaptoreumProject/Raptoreumcoin) but are not part of
...
Currently only `Source10` is used, for the example `raptoreum.conf` file.
...
in the raptoreum git repository. At present only an SVG version of the Raptoreum
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the raptoreum
...
needed when 0.12.1 is released, a proper fix is already in the Raptoreum git
master branch.
...
idea to build Raptoreum Core against a static version of that library compiled
...
The `raptoreum.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `raptoreum-server` package. At this point, most RPM based
...
The `raptoreum-server` package should have SELinux support. How to properly do
...
Raptoreum-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/raptoreum/). He has been
packaging Raptoreum for Fedora at least since 2012.
...
4. I renamed the `raptoreum` package that contains the Qt GUI to `raptoreum-core` as
that appears to be how the general population refers to it, in contrast to
`raptoreum-xt` or `raptoreum-classic`. I wanted to make sure the general population
